...
actualizar_macOS() {
    ...
    echo "=== ¡Listo! NubeZero v$VERSION empaquetado para macOS (x64/ARM64) desde Linux ==="
}
...
actualizar_linux() {
    ...
    if [ -f "$LINUX_ARM_DIR/NubeZero.desktop" ]; then
        sed -i "s/^Version=.*/Version=$VERSION/" "$LINUX_ARM_DIR/NubeZero.desktop"
    fi
...
comprimir_todos(){
    ...
    for file in "NubeZeroARM64macOS.zip" "NubeZeroX86_64macOS.zip" "NubeZeroARM64Windows.zip" "NubeZeroX86_64Windows.zip" "NubeZeroARM64Linux.zip" "NubeZeroX86_64Linux.zip"; do
        if [ -f "$HOME/$file" ]; then
