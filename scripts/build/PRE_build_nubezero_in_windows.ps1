<#
    NubeZero Build Script - Windows Edition
...
$LINUX_ARM_DIR = "$env:USERPROFILE\NubeZeroARM64Linux"

# Para macOS (solo se crean carpetas; el empaquetado no es posible en Windows)
$MACOS_ARM_APP = "$env:USERPROFILE\NubeZeroARM64macOS\NubeZero.app"
$MACOS_X64_APP = "$env:USERPROFILE\NubeZeroX86_64macOS\NubeZero.app"
...
function actualizar_macOS {
    ...
    Write-Host "=== ¡Listo! NubeZero v$VERSION empaquetado para macOS desde Windows ==="
...
function carpeta_linux {
    ...
    $desktopTemplate = "$PROJECT_DIR\src\desktop\NubeZero.desktop"
    if (Test-Path $desktopTemplate) {
        Copy-Item $desktopTemplate $LINUX_X64_DIR
        Copy-Item $desktopTemplate $LINUX_ARM_DIR
    } else {
        Write-Host "Aviso: No se encontró NubeZero.desktop base"
    }
...
function actualizar_linux {
    ...
    Write-Host "=== ¡Listo! NubeZero v$VERSION empaquetado para Linux (x64/ARM64) ==="
}
...
function comprimir_todos {
    ...
    $dirs = @(
        "NubeZeroARM64macOS",
        "NubeZeroX86_64macOS",
        "NubeZeroARM64Windows",
        "NubeZeroX86_64Windows",
        "NubeZeroARM64Linux",
        "NubeZeroX86_64Linux"
    )
