...
WIN_X64_DIR="$HOME/NubeZeroX86_64Windows/"
WIN_ARM_DIR="$HOME/NubeZeroARM64Windows/"
...
carpeta_linux() {
    ...
    cp "$PROJECT_DIR/src/desktop/NubeZero.desktop" "$LINUX_X64_DIR/" 2>/dev/null || echo "Aviso: No se encontró NubeZero.desktop base"
    cp "$PROJECT_DIR/src/desktop/NubeZero.desktop" "$LINUX_ARM_DIR/" 2>/dev/null
...
# Compilar para Linux x64
    ...
    cp "$PROJECT_DIR/src/desktop/Assets/icon.png" "$LINUX_ARM_DIR/"
    ...
    if [ -f "$LINUX_ARM_DIR/NubeZero.desktop" ]; then
        sed -i.bak "s/^Version=.*/Version=$VERSION/" "$LINUX_ARM_DIR/NubeZero.desktop"
        rm "$LINUX_ARM_DIR/NubeZero.desktop.bak"
    fi

    # Procesar versión X86_64
    cp -a "$PROJECT_DIR/src/desktop/bin/Release/net10.0/linux-x64/publish/." "$LINUX_X64_DIR/"
    cp "$PROJECT_DIR/src/desktop/Assets/icon.png" "$LINUX_X64_DIR/"
    
    if [ -f "$LINUX_X64_DIR/NubeZero.desktop" ]; then
        sed -i.bak "s/^Version=.*/Version=$VERSION/" "$LINUX_X64_DIR/NubeZero.desktop"
        rm "$LINUX_X64_DIR/NubeZero.desktop.bak"
    fi

    echo "=== ¡Listo! NubeZero v$VERSION empaquetado para Linux (x64/ARM64) ==="
}
...
comprimir_todos(){
    ...
    if [ -f "$HOME/NubeZeroARM64macOS.zip" ]; then
        echo "El archivo NubeZeroARM64macOS.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroARM64macOS.zip"
    fi
    if [ -f "$HOME/NubeZeroX86_64macOS.zip" ]; then
        echo "El archivo NubeZeroX86_64macOS.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroX86_64macOS.zip"
    fi
    if [ -f "$HOME/NubeZeroARM64Windows.zip" ]; then
        echo "El archivo NubeZeroARM64Windows.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroARM64Windows.zip"
    fi
    if [ -f "$HOME/NubeZeroX86_64Windows.zip" ]; then
        echo "El archivo NubeZeroX86_64Windows.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroX86_64Windows.zip"
    fi
    if [ -f "$HOME/NubeZeroARM64Linux.zip" ]; then
        echo "El archivo NubeZeroARM64Linux.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroARM64Linux.zip"
    fi
    if [ -f "$HOME/NubeZeroX86_64Linux.zip" ]; then
        echo "El archivo NubeZeroX86_64Linux.zip ya existe, eliminando..."
        rm "$HOME/NubeZeroX86_64Linux.zip"
    fi

    cd "$HOME" || exit 1
    for dir in NubeZeroARM64macOS NubeZeroX86_64macOS NubeZeroARM64Windows NubeZeroX86_64Windows NubeZeroARM64Linux NubeZeroX86_64Linux; do
        if [ -d "$dir" ]; then
            zip -r "${dir}.zip" "$dir"
        else
